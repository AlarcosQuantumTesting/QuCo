...
bp = Blueprint("run_qiskit_annealing", __name__)

def _run_qiskit_annealing_path() -> str:
    """Ruta absoluta a run_qiskit_annealing.py."""
    this_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.normpath(os.path.join(this_dir, "..", "run_qiskit_annealing.py"))
...
@bp.route("/run_qiskit_annealing", methods=["POST"])
def run_qiskit_annealing():
    ...
    import sys
    run_script = _run_qiskit_annealing_path()
...
@bp.route("/run_qiskit_annealing/status/<batch_id>", methods=["GET"])
def run_qiskit_annealing_status(batch_id: str):
...
@bp.route("/run_qiskit_annealing/get_file/<int:batch_id>/<filename>", methods=["GET"])
...
@bp.route("/run_qiskit_annealing/get_summary/<int:batch_id>", methods=["GET", "POST"])
def get_summary(batch_id: int):
    batch_dir = os.path.join(TEMP_SCRIPTS_DIR, str(batch_id))
    file_path = os.path.join(batch_dir, "summary.csv")
    if not os.path.exists(file_path):
        return jsonify({"error": "Summary no encontrado"}), 404
    return send_file(file_path, mimetype="text/csv")

@bp.route("/run_qiskit_annealing/get_results/<int:batch_id>", methods=["GET", "POST"])
def get_results(batch_id: int):
    batch_dir = os.path.join(TEMP_SCRIPTS_DIR, str(batch_id))
    file_path = os.path.join(batch_dir, "all_results.csv")
    if not os.path.exists(file_path):
        return jsonify({"error": "Results no encontrado"}), 404
    return send_file(file_path, mimetype="text/csv")

@bp.route("/run_qiskit_annealing/get_stdout/<int:batch_id>", methods=["GET", "POST"])
def get_stdout(batch_id: int):
    batch_dir = os.path.join(TEMP_SCRIPTS_DIR, str(batch_id))
    file_path = os.path.join(batch_dir, "stdout.txt")
    if not os.path.exists(file_path):
        return jsonify({"error": "Stdout no encontrado"}), 404
    return send_file(file_path, mimetype="text/plain")

@bp.route("/run_qiskit_annealing/get_stderr/<int:batch_id>", methods=["GET", "POST"])
def get_stderr(batch_id: int):
    batch_dir = os.path.join(TEMP_SCRIPTS_DIR, str(batch_id))
    file_path = os.path.join(batch_dir, "stderr.txt")
    if not os.path.exists(file_path):
        return jsonify({"error": "Stderr no encontrado"}), 404
    return send_file(file_path, mimetype="text/plain")
