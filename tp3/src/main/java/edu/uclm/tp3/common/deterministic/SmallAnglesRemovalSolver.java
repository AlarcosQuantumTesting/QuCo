...
public class SmallAnglesRemovalSolver extends Solver {

	private int depth;

	public SmallAnglesRemovalSolver(BinaryTree tree, Circuit circuit) {
		super(tree, circuit);
	}
	...
	public Map<String, Object> solve(int shots) throws IOException {
		...
		Map<Integer, String> usedNodesMap = new HashMap<>();
		for (int i = nodeNames.size() - 1; i >= 0; i--) {
			String nodeName = nodeNames.get(i);
			BinaryTree node = nodes.get(nodeName);
			int nodeDepth = node.getDepth();
			String code = node.getCode(nodeDepth, usedNodesMap);
			if (code != null)
				usedNodesMap.put(node.hashCode(), code);
		}
	...
	private void buildGates(BinaryTree node, int startQubit, int depth, List<String> gatesCode) {
		...
		if (node.rightChild == null) {
			...
			if (gatesCode.size() > sizePre) {
				...
				code.append("\tU.append(get" + node.leftChild.name + "(), ["
						+ this.getTargetQubits(1, depth - startQubit) + "])\n");
				code.append("\treturn U.to_gate()\n\n");
				gatesCode.add(code.toString());
			} else {
				code.append("\tU.ry(" + node.leftAngle + ", 0)\n");
		...
		if (node.leftChild == null) {
			...
			if (gatesCode.size() > sizePre) {
				...
				code.append("\tU.append(get" + node.rightChild.name + "(), ["
						+ this.getTargetQubits(1, depth - startQubit) + "])\n");
				code.append("\treturn U.to_gate()\n\n");
				gatesCode.add(code.toString());
			} else {
		...
		if (node.leftAngle == node.rightAngle) {
			...
			if (gatesCode.size() > sizePreLeft) {
				code.append("\tU.x(0)\n");
				code.append("\tU.append(get" + node.leftChild.name + "().control(1), ["
						+ this.getTargetQubits(0, depth - startQubit) + "])\n");
				code.append("\tU.x(0)\n");
				added = true;
			}
			...
			if (gatesCode.size() > sizePreRight) {
				code.append("\tU.append(get" + node.rightChild.name + "().control(1), ["
						+ this.getTargetQubits(0, depth - startQubit) + "])\n");
				added = true;
			}
	...
	private String getInitialize(Map<Integer, String> usedNodesMap) {
		String h = "";
		for (int i = 0; i < depth; i++)
			h = h + "circuit.h(" + i + ")\n";

		StringBuilder sbSubcircuits = new StringBuilder();
		for (String m : usedNodesMap.values())
			sbSubcircuits.append(m);
