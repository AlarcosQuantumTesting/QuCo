...
public class AddOrRemoveGate extends Strategy {
	...
	public AddOrRemoveGate(String gt, ProblemConfiguration pc, Fitnesser fitnesser, ManagerService manager) {
	...
	public void apply(String templateStart, String templateEnd) throws Exception {
		...
		for (int i=0; i<badCircuits.length; i++) {
			...
			if (columns>2) {
				...
				if (columns<pc.getInputConfiguration().getMaxNumberOfColumns()) {
					if (tirada<0.8)
